...
const Appointments: React.FC = () => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/appointments/user?email=${encodeURIComponent(parsedUser.email)}`);
      if (res.ok) {
  ...
  useEffect(() => { fetchAppointments(); }, [navigate, API_BASE_URL]);
  ...
  const submitCancellation = async () => {
    ...
    setIsCanceling(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/appointments/${cancelTargetId}/cancel`, {
  ...
  const submitDeletion = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/appointments/${deleteTargetId}`, { method: 'DELETE' });
